...
const usersColumns: ReadonlyArray<Column<any>> = [
  ...
  {
    Header: (props) => <CustomHeader tableProps={props} title='subdistrict_code' className='min-w-125px' />,
    id: 'subdistrict_code',
    Cell: ({...props}) => {
      const entry = props.data[props.row.index]
      return <ColumnString value={entry['subdistrict_code']} />
    },
  },
  {
    Header: (props) => <CustomHeader tableProps={props} title='Subdistrict Name' className='min-w-125px' />,
    id: 'name',
    Cell: ({...props}) => {
      const entry = props.data[props.row.index]
