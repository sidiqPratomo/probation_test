...
export const CreatePage: FC = () => {
  ...
  const onSubmit: SubmitHandler<Model> = async (data: Model) => {
    try {
      setIsLoading(true);
      await create<District>(`${Collection}`, data).then((response) => {
        const { data } = response;
        if (data) {
          enqueueSnackbar("District Created", {
            variant: "success",
          });
        }
        navigate(`/${Collection}`);
          ...
          <Form onSubmit={handleSubmit(onSubmit, onError)}>
            ...
            <Form.Group className="mb-4">
              ...
              <Form.Select
                ...
                disabled={!islandId && !provinceId} // Disable province selection until a region is selected
              >
